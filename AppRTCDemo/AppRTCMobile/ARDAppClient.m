...
- (void)peerConnection:(RTCPeerConnection *)peerConnection
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    __weak ARDAppClient *weakSelf = self;
    NSLog(@"😰😰😰😰😰😰😰😰😰😰 setLocalDescription self:%@ sdp:%@ ",self,sdp.sdp);
    ...
    NSLog(@"😰😰😰😰😰😰😰😰😰😰 sendSignalingMessage self:%@ sdp:%@ ",self,sdp.sdp);
    [self sendSignalingMessage:message];
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (!_isInitiator && !_peerConnection.localDescription) {
      ...
      __weak ARDAppClient *weakSelf = self;
    NSLog(@"😰😰😰😰😰😰😰😰😰😰 answerForConstraints  self:%@",self);
...
- (void)startSignalingIfReady {
  ...
  if (_isInitiator) {
    // Send offer.
    __weak ARDAppClient *weakSelf = self;
    NSLog(@"😰😰😰😰😰😰😰😰😰😰 offerForConstraints  self:%@",self);
...
- (void)processSignalingMessage:(ARDSignalingMessage *)message {
  ...
  switch (message.type) {
    ...
    case kARDSignalingMessageTypeAnswer: {
      ...
      __weak ARDAppClient *weakSelf = self;
     NSLog(@"😰😰😰😰😰😰😰😰😰😰 setRemoteDescription self:%@ sdp:%@ ",self,description.sdp);
...
- (void)setMaxBitrateForPeerConnectionVideoSender {
  for (RTCRtpSender *sender in _peerConnection.senders) {
      NSLog(@"😰😰😰😰😰😰😰😰😰😰 rtpSenderParam:%@,codecs:%@,name:%@",sender.parameters,sender.parameters.codecs,sender.parameters.codecs.firstObject.name);
      NSLog(@"😰😰😰😰😰😰😰😰😰😰 rtpSenderParam:%@,codecs:%@,name:%@",sender.parameters,sender.parameters.codecs,sender.parameters.codecs.firstObject.name);
    if (sender.track != nil) {
